...
LOCAL void ICACHE_FLASH_ATTR user_tcp_recv_cb(void *arg, char *pusrdata, unsigned short length)
{
    ...
    if(strcmp(pusrdata,"hello XXX")==0)
    {
    	os_printf("YES!");
    }else{
    	os_printf("NO!");
...
LOCAL void ICACHE_FLASH_ATTR user_dns_found(const char *name, ip_addr_t *ipaddr, void *arg)
{
    ...
    if (tcp_server_ip.addr == 0 && ipaddr->addr != 0)
   {
      // dns succeed, create tcp connection
        ...
        pespconn->proto.tcp->remote_port = 8088; // remote port of tcp server
...
void ICACHE_FLASH_ATTR user_check_ip(void)
{
    ...
    if (wifi_station_get_connect_status() == STATION_GOT_IP && ipconfig.ip.addr != 0)
   {
      ...
      os_printf("DNS ENABLE!!!!!");
       ...
       user_tcp_conn.proto.tcp->remote_port = 8088;  // remote port
       ...
       os_printf("satrt connect server....................!!!!!");
#endif
    }
   else
   {
